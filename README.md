...
- **子项维度**可在 `config.toml` 的 `[[dimensions]]` 里任意增删改，默认内置 **16 项**
  （混合养成 / 抽象 meme / RPG 属性）：熟悉度、信赖度、欢乐值、麻烦度、贴贴度、抽象度、
  含金量、电波、威胁度、慷慨度、智力、魅力、活跃、幸运、混沌、中二度。
  - 默认不在 config 里写维度即用内置 16 项，插件升级新增维度也能自动用上。
...
覆盖：能力声明与源码一致、配置一致性、维度解析（16 项）、生效配置回退、SVG 生成（雷达 + 量表条双向越界）、
